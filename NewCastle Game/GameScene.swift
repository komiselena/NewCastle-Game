...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let rotationSpeed: CGFloat = 0.03 // Скорость поворота
    ...
    override func didMove(to view: SKView) {
        ...
        backgroundTexture = SKTexture(imageNamed: gameViewModel?.backgroundImage ?? "BG1")
    ...
    func setupCoinDisplay() {
        ...
        let capsuleWidth: CGFloat = 90  // Немного уменьшил ширину
        ...
        capsule.fillColor = UIColor(hex: "#7C0A99") // Фиолетовый цвет
    ...
    func setupHeartsDisplay() {
        let heartSpacing: CGFloat = 30
        let totalWidth = CGFloat(maxHearts) * heartSpacing
        let startX = -totalWidth / 2 // Центрируем по горизонтали
        
        for i in 0..<maxHearts {
            let heart = SKSpriteNode(imageNamed: "heart")
            heart.size = CGSize(width: 25, height: 25)
            heart.position = CGPoint(x: startX + CGFloat(i) * heartSpacing, y: size.height/2.5)
    ...
    func updateHeartsDisplay() {
        for (index, heart) in hearts.enumerated() {
            heart.alpha = index < health ? 1.0 : 0.3
        }
    }
    ...
    func setupInfiniteBackground() {
        ...
        for i in 0..<3 {
            let bg = SKSpriteNode(imageNamed: gameViewModel?.backgroundImage ?? "BG1")
    ...
    func increaseHealth() {
        health = min(maxHearts, health + 1)
        
        // Анимация восстановления здоровья
        let heart = hearts[min(health-1, maxHearts-1)]
        let scaleUp = SKAction.scale(to: 1.5, duration: 0.2)
        let scaleDown = SKAction.scale(to: 1.0, duration: 0.2)
        heart.run(SKAction.sequence([scaleUp, scaleDown]))
    }
    ...
    func addBackgroundTile(at tilePos: CGPoint) {
        let texture = SKTexture(imageNamed: gameViewModel?.backgroundImage ?? "BG1")
    ...
    func spawnSkull() {
        ...
        let currentEnemies = children.filter { $0.name == "skull" }.count
        if currentEnemies >= maxEnemiesOnScreen {
            return
        }
        
        let bird = SKSpriteNode(imageNamed: "_Group_-4")
        bird.name = "skull"
        bird.zPosition = 5
        bird.setScale(1.7)
        
        bird.physicsBody = SKPhysicsBody(texture: bird.texture!, size: bird.size)
        bird.physicsBody?.categoryBitMask = PhysicsCategory.skull
               ...
               let rubin = secondBody.node as? SKSpriteNode {
                ...
                if currentRubin == rubin {
                    ...
                    rubinSpawnTimer = 0
                        ...
                        secondBody.categoryBitMask == PhysicsCategory.skull {
                reduceHealth()
